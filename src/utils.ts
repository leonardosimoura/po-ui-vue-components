...
export function formatBytes(bytes: number, decimals = 2): string {
    ...
    return `${parseFloat(
        (bytes / Math.pow(multiplier, result)).toFixed(decimals)
    )} ${sizes[result]}`;
}
...
export function convertToInt(value: any, valueDefault?: any): number {
    ...
    const defaultValue =
        validDefaultValue || validDefaultValue === 0
            ? validDefaultValue
            : undefined;

    return validNumber || validNumber === 0
        ? validNumber
 ...
 */
export function callFunction(fn: any, context: any, param?: any): void {
...
export function isEquals(value: any, comparedValue: any) {
...
export function sortFields(fields = [], defaultOrdering = -1) {
    ...
    const isOrderValid = (order: number) =>
        isTypeof(order, "number") && order > 0;
    const applyDefaultOrdering = (order: number) =>
        isOrderValid(order) ? order : defaultOrdering;

    return fields.sort((fieldA: any, fieldB: any) => {
...
export function sortValues(
    ...
    const right = isTypeof(rightSide, "string")
        ? rightSide.toLowerCase()
        : rightSide;
 ...
 * @returns Array<any>
 */
export function mapObjectByProperties(
    object: any = {},
    properties: Array<string> = []
) {
    const getSelectedProperties = (selectedProperties: any, property: any) => ({
        ...selectedProperties,
...
export function convertNumberToDecimal(
    ...
    const floatValue = isValidValue ? parseFloat(number) : 0;
 ...
 * @param dirtyObject
 */
export function clearObject(dirtyObject: any) {
...
export function validateObjectType(value: object) {
